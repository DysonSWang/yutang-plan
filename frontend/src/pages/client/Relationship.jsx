...
/** ===================== 缘分 Tab ===================== */
function PondTab({ girlsList, isInitialLoad, onAddGirl, onGirlClick }) {
...
/** ===================== 约会 Tab ===================== */
function DatesTab({ datesList, allDates, pendingInterviews, isInitialLoad, onRefresh, onOpenDate, onDeleteDate, onOpenInterview }) {
              ...
              <Flex align="center" gap={6} wrap="wrap">
                ...
                <Box flex={1}>
                  <HStack spacing={2} mb={1}>
                    <Avatar size="sm" name={upcomingDate.girl?.name} src={upcomingDate.girl?.avatar ? getMediaUrl(upcomingDate.girl.avatar) : null} />
      ...
      {isInitialLoad ? (
        ...
        <VStack spacing={4} align="stretch">
          <Flex gap={3} wrap="wrap">
            <Select placeholder={`全部女生 (${datesList.length})`} w="180px" size="sm" bg="warm.800" color="white" borderColor="warm.600" value={filterGirlId} onChange={e => setFilterGirlId(e.target.value)}>
              {[] /* TODO: fill with girl list */}
            </Select>
            ...
            <Card key={d.id} bg="warm.800" border="1px solid" borderColor="purple.500"
              ...
              <CardBody py={4} px={5}>
                <Flex gap={4} align="flex-start">
                  <Avatar size="lg" name={d.girl?.name} src={d.girl?.avatar ? getMediaUrl(d.girl.avatar) : null} />
                  <Box flex={1}>
...
export default function Relationship() {
  ...
  // 默认 Tab：缘分=0，约会=1，日历=2
  const defaultTab = searchParams.get('tab') === 'calendar' ? 2 : searchParams.get('tab') === 'dates' ? 1 : 0;
  ...
  const handleGenerateWithPlan = async () => {
    if (!selectedGirlForDate) { toast({ title: '请先选择约会对象', status: 'warning' }); return; }
    setGenerating(true);
    try {
      const res = await dates.create({ girlId: selectedGirlForDate.id, dateTime: dateForm.dateTime || undefined, location: dateForm.location, title: dateForm.title || '新约会', notes: dateForm.notes });
      if (res.success) {
        ...
        resetDateForm();
      } else { toast({ title: res.error || '创建失败', status: 'error' }); }
    } catch (e) { toast({ title: '创建失败', status: 'error' }); }
    setGenerating(false);
  };
  ...
  return (
    <Box>
      ...
      <Tabs colorScheme="gold" variant="soft-rounded" defaultIndex={defaultTab} isLazy lazyBehavior="keepMounted">
        <TabList>
          <Tab>缘分</Tab>
          <Tab>约会</Tab>
          <Tab>日历</Tab>
        ...
        <TabPanels>
          ...
          <TabPanel px={0}>
            <DatesTab
              ...
              onDeleteDate={(id) => { /* TODO */ }}
              onOpenInterview={(iv) => { /* TODO */ }}
            />
          </TabPanel>
          <TabPanel px={0}>
