...
- **【强制格式要求】每个章节必须用空行分隔**：
  ...
  - 段落之间必须有一个空行（不要连在一起）
  - 列表项：\`- 列表内容\`（减号后必须加空格）
  - 加粗：**只能包裹关键词/短语（2-5个字），禁止包裹整句或整段**
  - 示例：**不要**这样写：\`**这段话全都被加粗了**\`
  - 示例：**正确**这样写：关键点是**情绪共鸣**，而不是**逻辑说服**
  - 标题和内容之间必须换行，不能写成 \`##标题\` 或 \`## 标题内容紧跟\`
  - 每个主要分析点之间用空行分隔，方便阅读
`.trim();
}
...
function buildOutputStyleInstruction(context) {
  ...
  lines.push('');
  lines.push('【Markdown格式强制要求】');
  lines.push('- 二级标题：## 标题（井号后必须加空格）');
  lines.push('- 段落之间必须有空行，不要写成连续段落');
  lines.push('- 列表项：- 内容（减号后必须加空格）');
  lines.push('- 加粗：**文字**，禁止单个星号');
  lines.push('- 标题后必须换行，不能写成 ##标题 或 ##标题内容');
  lines.push('- 章节之间必须用空行分隔（两个换行符）');
