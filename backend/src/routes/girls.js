...
router.get('/:id', authMiddleware, asyncHandler(async (req, res) => {
  ...
  return success(res, { girl: { ...girl, chatLogs, dates } });
}));
...
router.post('/', authMiddleware, asyncHandler(async (req, res) => {
  ...
  if (currentCount >= quota) {
    throw new AppError(ErrorCodes.CLIENT_QUOTA_EXCEEDED);
  }
...
// 更新亲密度
router.post('/:id/intimacy', authMiddleware, asyncHandler(async (req, res) => {
  if (req.user.role !== 'operator' && req.user.role !== 'admin') {
    throw new AppError(ErrorCodes.AUTH_PERMISSION_DENIED);
  }
...
// 客户自己添加女生（带额度校验）
router.post('/client-add', authMiddleware, asyncHandler(async (req, res) => {
  if (req.user.role !== 'client') {
    throw new AppError(ErrorCodes.AUTH_PERMISSION_DENIED);
  }
  ...
  return success(res, { girl, quotaLeft: quota - currentCount - 1 });
}));
...
// 获取关系阶段变更历史
router.get('/:id/stage-history', authMiddleware, asyncHandler(async (req, res) => {
  const { error } = await checkGirlOwnership(req.params.id, req.user);
  if (error) throw new AppError(ErrorCodes.AUTH_PERMISSION_DENIED);
...
router.post('/:id/evaluate-stage', authMiddleware, asyncHandler(async (req, res) => {
  if (req.user.role !== 'operator' && req.user.role !== 'admin') {
    throw new AppError(ErrorCodes.AUTH_PERMISSION_DENIED);
  }
  const { error } = await checkGirlOwnership(req.params.id, req.user);
...
// 手动设置关系阶段
router.put('/:id/relationship-stage', authMiddleware, asyncHandler(async (req, res) => {
  if (req.user.role !== 'operator' && req.user.role !== 'admin') {
    throw new AppError(ErrorCodes.AUTH_PERMISSION_DENIED);
  }
  const { error } = await checkGirlOwnership(req.params.id, req.user);
  ...
  if (!VALID_STAGES.includes(stage)) {
    throw new AppError(ErrorCodes.GIRL_STAGE_INVALID);
  }
