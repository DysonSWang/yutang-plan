...
async function callVisionModel(messages, vlConfig) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`VL 模型调用失败: ${response.status} ${errorText}`);
  }
...
只输出 JSON，不要其他内容。profileUpdates 只填充空字段或发现变化的字段，不要编造。`;
...
const GIRL_FIELD_LABELS = {
  ...
  appearance: '外貌', dressingStyle: '穿着风格', styleTags: '风格', familyBackground: '家庭背景',
